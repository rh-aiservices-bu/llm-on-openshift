...
def get_pdf_file(session_id):
    return os.path.join("./assets", PDF_FILE_DIR, 'proposal_{session_id}.pdf')
...
def generate_session_id():
    ...
    session_id = f"{timestamp}_{random_number}"
    return session_id
...
with gr.Blocks(title="HatBot") as demo:
    ...
    def update_output(customer, product):
        content, session_id = "", ""
        for c, sid in ask_llm(customer, product):
            content = c
            session_id = sid
        pdf_file_path = get_pdf_file(session_id)
        return content, pdf_file_path
        
            
    download_button.click(lambda: [], inputs=[])
    submit_button.click(update_output, inputs=[customer_box, product_dropdown], outputs=[output_answer, download_button])
    ...
    @gr.update
    def update_download_button(pdf_file_path):
        if pdf_file_path:
            download_button.set_clickable(True)
            download_button.change(label="Download PDF", value=pdf_file_path)
        else:
            download_button.set_clickable(False)
            download_button.change(label="Download PDF", value="")
